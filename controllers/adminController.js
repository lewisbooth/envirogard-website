...
exports.newProductSave = async (req, res, next) => {
  ...
  await new Product(product).save((err, data) => {
...
exports.editProductSave = async (req, res, next) => {
  ...
  await Product.findOneAndUpdate(
    ...
    { new: true },
    (err, doc) => {
      if (err || !doc) return res.status(400).send();
...
exports.uploadProductManual = async (req, res, next) => {
  ...
  const newPDF = req.files.filter(
    (file) =>
      file.fieldname === "manualPDF" && file.mimetype === "application/pdf"
  )[0];
...
exports.newCategorySave = async (req, res, next) => {
  ...
  await new Category(category).save((err, data) => {
    if (err) return res.status(400).send();
    // Attach document to 'req' to be used by the file upload handlers
    req.category = data;
...
exports.deleteCategory = async (req, res) => {
  await Category.findByIdAndRemove(req.params.id, (err, doc) => {
    if (err) {
      console.log(err);
...
exports.uploadCategoryImage = async (req, res, next) => {
  ...
  if (
    (newImage || req.body.deleteImage === "true") &&
    fs.existsSync(`${folder}/cover.jpg`)
  ) {
...
exports.newSubcategorySave = async (req, res, next) => {
  ...
  await new Subcategory(subcategory).save(async (err, doc) => {
    if (err) {
      console.log(err);
    ...
    async (err, doc) => {
      ...
      if (subcategory.products) {
        subcategory.products.forEach(async (_id) => {
          await Product.updateMany({ _id }, { $set: { subcategory: doc._id } });
        });
      }
      // Manually call save() to trigger slug update
      doc.save();
...
exports.deleteSubcategory = async (req, res) => {
  await Subcategory.findByIdAndRemove(req.params.id, async (err, doc) => {
    if (err) {
      console.log(err);
...
exports.newIndustrySave = async (req, res, next) => {
  ...
  await new Industry(industry).save((err, data) => {
    if (err) return res.status(400).send();
...
exports.deleteIndustry = async (req, res) => {
  await Industry.findByIdAndRemove(req.params.id, (err, doc) => {
    if (err) {
      console.log(err);
...
exports.editIndustry = async (req, res) => {
  const industry = await Industry.findOne({ slug: req.params.slug }).populate(
    "subcategories"
  );
  if (!industry) {
